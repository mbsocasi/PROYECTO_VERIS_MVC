...
namespace PROYECTO_VERIS_MVC.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    using (ProyectoVeris_MVC_BDEntities db = new ProyectoVeris_MVC_BDEntities())
                    {
                        var usuario = from u in db.AspNetUsers
                ...
                if (result.Succeeded)
                {
                    ...
                    using (ProyectoVeris_MVC_BDEntities db = new ProyectoVeris_MVC_BDEntities())
                    {
                        var usuario = from u in db.AspNetUsers
                                      where u.UserName == model.UserName
                                      select u;
                        var id = usuario.FirstOrDefault().Id;
                        if (usuario != null)
                        {
                            Session["usuario"] = usuario.FirstOrDefault();
        ...
        //Inicio de secion para esconder login
        [AllowAnonymous]
